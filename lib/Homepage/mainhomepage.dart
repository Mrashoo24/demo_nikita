...
class MainHomePage extends StatefulWidget {
  ...
  final function;
  const MainHomePage({Key? key, this.userModel, required this.function})
      : super(key: key);
    ...
    return SingleChildScrollView(
      child: FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            var present =
                attendencecount.where((element) => element.status != 'absence');

            var Absent =
                attendencecount.where((element) => element.status == 'absence');

            var lateentry =
                attendencecount.where((element) => element.status == 'late');

            var earlyexit = attendencecount
                .where((element) => element.workingstatus == 'early');
            ...
            if (report != "No Data") {
              ...
              dateAndTime = DateFormat('hh:mm a').parse(checkInTime);
              ...
              convertedTime =
                  double.parse(newTimeList[0] + '.' + newTimeList[1]);

              differenceintime = (differenceTime.inMinutes /
                      (double.parse(widget.userModel!.hoursOfShift!) * 60)) /
                  2;

              gotreason = report == "No Data" ? "-----" : report["reason"];

              print(differenceTime.inMinutes);
              print('difference in time $differenceintime');
              ...
              widthofcontainer =
                  ...
                  differenceintime > 0.44
                      ? Get.width * 0.45
                      : Get.width * differenceintime;

              print('width = $widthofcontainer');

              print(
                  'calculation ${(Get.width - 55) - ((Get.width - 55) - (widthofcontainer))}');
                    ...
                    child: Stack(
                      children: [
                        ...
                        Positioned(
                          ...
                          //(Get.width - 55) - ((Get.width - 55) - (widthofcontainer)) + widthofcontainer ,
                          // differenceintime > 0.43 ? differenceintime + Get.width *0.37 : differenceintime + Get.width *0.48 ,
                          child: Padding(
                                ...
                                Padding(
                                  ...
                                  child: Text(
                                    convertedTime
                                        ...
                                        .replaceAll('.', ':'),
                                    style:
                                        TextStyle(color: kgolder, fontSize: 20),
                                  ),
                                )
                              ],
                            ),
